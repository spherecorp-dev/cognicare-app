...
const SUPPORTED_AGENTS = {
  ...
  'stefan-georgi': {
    name: 'stefan-georgi',
    displayName: 'Stefan Georgi - DR Copywriter',
    file: 'stefan-georgi.md',
    capabilities: ['copywriting', 'direct-response', 'angles', 'scripts', 'ad-copy'],
    squadLookup: true,
  },
  'copy-chief': {
    name: 'copy-chief',
    displayName: 'Copy Chief - Creative Quality Director',
    file: 'copy-chief.md',
    capabilities: ['review', 'strategy', 'data-interpretation', 'quality-control'],
    squadLookup: true,
  },
  'media-head': {
    name: 'media-head',
    displayName: 'Media Head - Director de Mídia',
    file: 'media-head.md',
    capabilities: ['media-strategy', 'budget-allocation', 'campaign-planning'],
    squadLookup: true,
  },
  'media-analyst': {
    name: 'media-analyst',
    displayName: 'Media Analyst - Analista de Performance',
    file: 'media-analyst.md',
    capabilities: ['performance-analysis', 'reporting', 'optimization'],
    squadLookup: true,
  },
  'media-buyer': {
    name: 'media-buyer',
    displayName: 'Media Buyer - Comprador de Mídia',
    file: 'media-buyer.md',
    capabilities: ['campaign-execution', 'ad-buying', 'scaling'],
    squadLookup: true,
  },
  'media-engineer': {
    name: 'media-engineer',
    displayName: 'Media Engineer - Engenheiro de Tracking',
    file: 'media-engineer.md',
    capabilities: ['tracking', 'pixels', 'utm', 'attribution'],
    squadLookup: true,
  },
};
...
class AgentInvoker extends EventEmitter {
   ...
   * @param {LLMService} [options.llmService] - LLM Service instance for real execution
   */
  constructor(options = {}) {
    super();
    ...
    // LLM Service — lazy init if API key available
    this.llmService = options.llmService || null;
    if (!this.llmService && process.env.ANTHROPIC_API_KEY) {
      try {
        this.llmService = new LLMService();
      } catch {
        // No API key — will run in simulated mode
      }
    }
    ...
    this.squadsDir = path.join(this.projectRoot, 'squads');
   ...
   *
   * @param {string} agentName - Agent name (e.g., 'dev', 'qa', 'stefan-georgi')
   * @param {string} taskPath - Path to task file or task name
   * @param {Object} [inputs={}] - Inputs to pass to the task
   * @param {Object} [options={}] - Additional options
   * @param {string} [options.squadName] - Squad name for squad-specific agent/task lookup
   * @returns {Promise<Object>} Invocation result
   */
  async invokeAgent(agentName, taskPath, inputs = {}, options = {}) {
    const invocationId = this._generateId();
    const startTime = Date.now();
    const squadName = options.squadName || null;

    this._log(`Invoking @${agentName} for ${taskPath}${squadName ? ` (squad: ${squadName})` : ''}`, 'info');
    ...
    try {
      // Validate agent (AC2) — squad-aware lookup
      const agent = await this._loadAgent(agentName, squadName);
      if (!agent) {
        throw new Error(`Unknown agent: ${agentName}`);
      }

      // Load task — squad-aware lookup
      const task = await this._loadTask(taskPath, squadName);
      if (!task) {
        throw new Error(`Task not found: ${taskPath}`);
      }

      // Build context (AC3)
      const context = this._buildContext(agent, task, inputs, options);
  ...
  /**
   * Load agent definition (AC2) — squad-aware lookup
   *
   * Search order for squad agents:
   * 1. squads/{squadName}/agents/{agent}.md
   * 2. .aios-core/development/agents/{agent}.md (framework agents)
   * 3. Fallback: return config without persona content
   *
   * @param {string} agentName - Agent name
   * @param {string} [squadName] - Squad name for squad-specific lookup
   * @private
   */
  async _loadAgent(agentName, squadName) {
    ...
    // Build search paths — squad-specific first, then framework
    const searchPaths = [];
    if (squadName && agentConfig.squadLookup) {
      searchPaths.push(path.join(this.squadsDir, squadName, 'agents', agentConfig.file));
    }
    searchPaths.push(path.join(this.agentsDir, agentConfig.file));
    ...
    this._log(`Agent file not found for ${name} in: ${searchPaths.join(', ')}`, 'warn');
    // Return config without file content — allows execution with generic persona
    return {
      ...agentConfig,
      loaded: false,
      content: null,
  ...
  /**
   * Load task definition — squad-aware lookup
   *
   * Search order:
   * 1. squads/{squadName}/tasks/{task}.md (if squadName provided)
   * 2. .aios-core/development/tasks/{task}.md (framework tasks)
   * 3. Absolute/relative path fallback
   *
   * @param {string} taskPath - Task name or path
   * @param {string} [squadName] - Squad name for squad-specific lookup
   * @private
   */
  async _loadTask(taskPath, squadName) {
    // Handle absolute paths directly
    if (path.isAbsolute(taskPath)) {
      if (await fs.pathExists(taskPath)) {
        const content = await fs.readFile(taskPath, 'utf8');
        const metadata = this._parseTaskMetadata(content);
        return { path: taskPath, name: path.basename(taskPath, '.md'), content, ...metadata };
      }
    ...
    if (squadName) {
      searchPaths.push(path.join(this.squadsDir, squadName, 'tasks', taskName));
    }
    searchPaths.push(path.join(this.tasksDir, taskName));
    searchPaths.push(path.join(this.projectRoot, taskPath));

    for (const fullPath of searchPaths) {
      if (await fs.pathExists(fullPath)) {
        const content = await fs.readFile(fullPath, 'utf8');
        const metadata = this._parseTaskMetadata(content);
        return { path: fullPath, name: path.basename(fullPath, '.md'), content, ...metadata };
      }
    }

    this._log(`Task file not found: ${taskName} in: ${searchPaths.join(', ')}`, 'warn');
    return null;
  }
   ...
   * @private
   */
  _buildContext(agent, task, inputs, options = {}) {
    return {
      // Agent info
      agent: {
        name: agent.name,
      ...
      // Per-step LLM options (maxTokens, temperature) from pipeline config
  ...
  /**
   * Execute task (AC4) — LLM-powered execution
   *
   * Priority: custom executor → LLMService → simulated fallback
   * @private
   */
  async _executeTask(agent, task, context) {
    // Per-step timeout override via llm_options.timeout (default: 5 min)
    const timeout = context.llmOptions?.timeout || this.defaultTimeout;
    ...
    // Real LLM execution via LLMService
    if (this.llmService) {
      return await this._executeWithTimeout(
        () => this._executeLLMTask(agent, task, context),
        timeout,
      );
    }

    // Fallback: simulated result (no API key configured)
    this._log(`No LLM service available — returning simulated result for ${task.name}`, 'warn');
    return {
      status: 'simulated',
      message: `Task ${task.name} executed by @${agent.name} (no ANTHROPIC_API_KEY)`,
      context: {
   ...
   *
   * Builds system prompt from agent persona, task instructions from task .md file,
   * and input data from pipeline context → calls Claude → returns structured output.
   *
   * @param {Object} agent - Loaded agent config with persona content
   * @param {Object} task - Loaded task with .md content
   * @param {Object} context - Built context with inputs
   * @returns {Promise<Object>} Parsed LLM output
   * @private
   */
  async _executeLLMTask(agent, task, context) {
    // Build system prompt from agent persona
    const systemPrompt = this._buildSystemPrompt(agent);

    // Task instructions come from the .md file content
    const taskInstructions = task.content || '';

    // Input is the pipeline context inputs
    const input = context.inputs || {};

    // Per-step LLM options (maxTokens, temperature) from pipeline step config
    const llmOptions = context.llmOptions || {};

    this._log(`Calling LLM: @${agent.name} → ${task.name} (input keys: ${Object.keys(input).join(', ')})`, 'info');
    ...
    const result = await this.llmService.execute({
      ...
      input,
      outputFormat: 'json',
      ...(llmOptions.maxTokens && { maxTokens: llmOptions.maxTokens }),
      ...(llmOptions.temperature != null && { temperature: llmOptions.temperature }),
    });

    this._log(`LLM response: ${result.model}, ${result.usage?.input_tokens || 0}+${result.usage?.output_tokens || 0} tokens, continuations: ${result.continuations || 0}`, 'info');

    // Detect _parseError — LLM returned unparseable output
    if (result.output && result.output._parseError) {
      const rawPreview = (result.output._raw || '').slice(0, 200);
      throw new Error(
        `LLM output parse failed for @${agent.name}/${task.name}: ` +
        `response was ${result.usage?.output_tokens || '?'} tokens, ` +
        `${result.continuations || 0} continuations. ` +
  ...
  /**
   * Build system prompt from agent persona .md file
   *
   * If agent has loaded persona content, uses it as system prompt.
   * Otherwise builds a minimal generic prompt from agent config.
   *
   * @param {Object} agent - Agent config with optional content
   * @returns {string} System prompt
   * @private
   */
  _buildSystemPrompt(agent) {
    if (agent.loaded && agent.content) {
      return agent.content;
    }

    // Generic fallback prompt
    return `You are @${agent.name} (${agent.displayName}), an AI agent specialized in: ${(agent.capabilities || []).join(', ')}. Respond with valid JSON only.`;
  }
  ...
  /**
   * Check if error is transient (retryable)
   * Includes common API rate limit and network errors
   * @private
   */
  _isTransientError(error) {
    const transientPatterns = [
      ...
      /temporary/i,
      /overloaded/i,
      /parse.?fail/i,    // JSON parse failure on continuation — retry may produce cleaner output
      /529/,
      /503/,
      /504/,
    ];
